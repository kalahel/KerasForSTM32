...
int main(void) {
	...
	int entries_number = -1;
	// Receive number of data entry and send it back for acknowledgement
	HAL_UART_Receive(&huart2, (uint8_t*) (&entries_number), sizeof(int), 1000000);
	if (entries_number < 0) {
		// Todo handle error
		printf("Error on number of data entries\r\n");
		return 1;
	}
	HAL_UART_Transmit(&huart2, (uint8_t*) (&entries_number),
			sizeof(int), 1000000);
	for (int entry_id = 0; entry_id < entries_number; ++entry_id) {

		HAL_UART_Receive(&huart2, (uint8_t*) in_data,
				AI_MNISTSMALL_IN_1_SIZE_BYTES, 1000000);
		// Computing and sending checksum of received data
		float checksum = 0.0;
		for (int i = 0; i < AI_MNISTSMALL_IN_1_SIZE; ++i) {
			checksum += ((float*) in_data)[i];
		}
		HAL_UART_Transmit(&huart2, (uint8_t*) &checksum, sizeof(float),
				1000000);

		ai_i32 nb_batches_treated = ai_mnistsmall_run(mnist_model, inputs,
				ouputs);

		// This is used to visualize easily our outputs using the debugger
		...
		HAL_UART_Transmit(&huart2, (uint8_t*) simple_outputs,
		AI_MNISTSMALL_OUT_1_SIZE_BYTES, 1000000);
	}
