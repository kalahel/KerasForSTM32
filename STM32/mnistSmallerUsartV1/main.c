...
int main(void) {
	...
	char buf_tx[AI_MNISTSMALL_OUT_1_SIZE_BYTES] = { 0 };

	HAL_UART_Receive(&huart2, (uint8_t*) in_data, AI_MNISTSMALL_IN_1_SIZE_BYTES,
			1000000);
	...
	HAL_UART_Transmit(&huart2, (uint8_t*) &checksum, sizeof(float), 1000000);

	ai_i32 nb_batches_treated = ai_mnistsmall_run(mnist_model, inputs, ouputs);
