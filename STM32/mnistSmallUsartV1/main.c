...
int main(void)
{
	/* USER CODE BEGIN 1 */
		...
		HAL_UART_Transmit(&huart2, (uint8_t*) &checksum,
			sizeof(float), 1000000);
		ai_i32 nb_batches_treated = ai_mnistsmall_run(mnist_model, inputs, ouputs);
		// WARNING RUNNING THE MODEL SEEMS TO RENDER ALL UART USAGE IMPOSSIBLE, NEED TO INVESTIGATE
		...
		float send_values[10] = { 2.2, 6.3, 4.54, 9.65, 1.2, 6.333, 7.6545, 88.22,
				0.66, 0.48 };
		for (int i = 0; i < 10 * sizeof(float); ++i) {
			buf_tx[i] = ((char*) send_values)[i];
		}
		HAL_UART_Transmit(&huart2, (uint8_t*) buf_tx,
